...
// onLoginSuccess now receives (mustChangePassword: boolean) — AuthGate uses
// this to decide whether to show the dashboard or the forced password screen.
export default function LandingPage({ onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    setError("")
    const res = await login(username, password) // now hits the real backend
    if (res.ok) {
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <svg width="640" height="640" viewBox="0 0 640 640"
          ...
          <defs>
            <pattern id="hex" width="58" height="100.5" patternUnits="userSpaceOnUse" patternTransform="scale(1)">
              ...
              <path d="M29 67 L58 83.75 L58 117.25 L29 134 L0 117.25 L0 83.75 Z" fill="none" stroke={C.cyan} strokeWidth="1.5" />
            </pattern>
          </defs>
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 10 }}>
          <img src={logo} alt={BRAND.parent} style={{ width: 34, height: 34, borderRadius: 9, objectFit: "cover", boxShadow: "0 0 0 1px rgba(255,255,255,0.15)" }} />
          <div style={{ fontSize: 13, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(255,255,255,0.6)" }}>{BRAND.parent}</div>
        </div>

        <div style={{ position: "relative", maxWidth: 460 }}>
          <div style={{ fontSize: 12, letterSpacing: "0.18em", textTransform: "uppercase", color: C.pink, marginBottom: 18, fontWeight: 600 }}>Outreach, catalogued</div>
          <h1 style={{ fontFamily: "'Fraunces', Georgia, serif", fontStyle: "italic", fontWeight: 500, fontSize: "clamp(40px, 5vw, 60px)", lineHeight: 1.04, margin: "0 0 20px", letterSpacing: "-0.01em" }}>{BRAND.name}</h1>
          <p style={{ fontSize: 16, lineHeight: 1.6, color: "rgba(255,255,255,0.72)", margin: "0 0 36px", fontWeight: 400 }}>{BRAND.tagline} One dashboard to find leads, write to them, and know exactly who opened the door.</p>

          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {SPECIMENS.map((s, i) => (
              <div key={s.tag} style={{ display: "flex", alignItems: "baseline", gap: 14, animation: `floatUp 0.5s ease ${0.1 + i * 0.08}s both`, paddingBottom: 14, borderBottom: i < SPECIMENS.length - 1 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
        ...
        <div style={{ position: "relative", fontSize: 12, color: "rgba(255,255,255,0.4)" }}>Built for {BRAND.parent} · by <span style={{ color: C.pink }}>{BRAND.author}</span></div>
      </div>

      <div style={{ flex: "0.85", display: "flex", alignItems: "center", justifyContent: "center", padding: 32, minWidth: 380 }}>
        <form onSubmit={handleSubmit} className={shake ? "nectar-shake" : ""} style={{ width: "100%", maxWidth: 340, background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: "36px 32px", boxShadow: "0 24px 60px rgba(27,31,39,0.06)" }}>
          <img src={logo} alt={BRAND.parent} style={{ width: 40, height: 40, borderRadius: 11, marginBottom: 20, objectFit: "cover", boxShadow: `0 0 0 1px ${C.border}` }} />
          ...
          <input className="nectar-input" value={username} onChange={e => { setUsername(e.target.value); setError("") }}
            ...
            style={{ width: "100%", boxSizing: "border-box", padding: "11px 14px", marginBottom: 16, borderRadius: 9, border: `1px solid ${error ? C.red : C.border}`, fontSize: 14, fontFamily: "inherit", color: C.text, transition: "border-color .15s" }} />
          ...
          <input ref={pwRef} className="nectar-input" type="password" value={password} onChange={e => { setPassword(e.target.value); setError("") }} placeholder="••••••••"
            style={{ width: "100%", boxSizing: "border-box", padding: "11px 14px", marginBottom: error ? 10 : 22, borderRadius: 9, border: `1px solid ${error ? C.red : C.border}`, fontSize: 14, fontFamily: "inherit", color: C.text, transition: "border-color .15s" }} />

          {error && <div style={{ background: C.redDim, color: C.red, fontSize: 12, padding: "8px 12px", borderRadius: 7, marginBottom: 16 }}>⚠ {error}</div>}
          ...
          <div style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: C.textDim }}>Session ke saath signed in — 7 din tak dobara login nahi maangega.</div>
        </form>
      </div>
    </div>
  )
